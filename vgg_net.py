...
class VGGNetFeat(object):
    ...
    def add_design_feature(self, controller):
        """ This function will extract the features using VGG-19 architecture
        :param db_object: A object for handling database operation
        :param db_conn: A connection to sqlite database
        """
        data = controller.get_train_image_path()
        with controller.conn:
            for path in data:
                print(path)
                d_hist = self.run_inference(path[0])
                feature = (d_hist, path[0])
                with open('data.csv', 'a') as f:
                    f.writelines(path[0] + ', ' + str(d_hist) + '\n')
                controller.update_features(feature)

    def generate_query(self, controller, image_path, root_path):
        """
        This function will generate a query for searching in database
        :param image_root: This is the root location of image
        :param distance: The type of distance function to run query
        :return: A dictionary for querying the database
        """
        with controller.conn as conn:
            data, flag = controller.check_in_database(image_path, root_path)
            if flag:
                query = {
                    'image_path': data[2],
                    'features': data[5]
                ...
                feature_vector = self.run_inference(image_path)
                feature = (feature_vector, image_path)
                controller.update_features(feature)
...
if __name__ == "__main__":
    ...
    import time
    net = VGGNetFeat()
    filename = "D:\\Work\\images\\floral.jpg"
    t1= time.time()
    features = net.run_inference(filename)
    print(time.time()-t1)
    print(features)